...
class FigureValidator:
    ...
    def check_array(self, arr, label: str, allow_negative: bool = True):
        ...
        if len(finite) > 0:
            print(f"    [STATS] {label}: min={np.min(finite):.4g}, max={np.max(finite):.4g}, "
    ...
    def check_chi2(self, chi2_reduced: float):
        ...
        if chi2_reduced < 0:
            ...
            self.issues.append(f"[WARN] χ²/dof = {chi2_reduced:.2f} is suspiciously low (overfitting?)")
        elif chi2_reduced > 100:
            self.issues.append(f"[WARN] χ²/dof = {chi2_reduced:.2f} is very high (poor fit)")
        else:
    ...
    def check_mcmc_convergence(self, sampler, flat_chain):
        ...
        try:
            ...
            if min_ratio < 10:
                self.issues.append(f"[WARN] Chain may not be converged: n_steps/τ = {min_ratio:.1f} < 10")
            else:
        ...
        if flat_chain is not None:
            ...
            if n_eff < 100:
                self.issues.append(f"[WARN] Only {n_eff} effective samples (want > 1000)")
            else:
    ...
    def check_parameter_bounds(self, params: FRBParams, log_space: bool = True):
        ...
        if log_space:
            ...
            if abs(params.c0) > 10:
                self.issues.append(f"[WARN] log(c0) = {params.c0:.4g} is extreme (|x| > 10)")
                
            # zeta: log(pulse width), reasonable for ms timescale
            if params.zeta < -15 or params.zeta > 5:
                self.issues.append(f"[WARN] log(zeta) = {params.zeta:.4g} outside reasonable range")
                
            # tau_1ghz: log(scattering time)
            if params.tau_1ghz < -15 or params.tau_1ghz > 5:
                self.issues.append(f"[WARN] log(tau_1ghz) = {params.tau_1ghz:.4g} outside reasonable range")
                
            # alpha is NOT in log-space, check typical range (3-5 for Kolmogorov)
            alpha = getattr(params, 'alpha', 4.0)
            if alpha < 1 or alpha > 8:
                self.issues.append(f"[WARN] alpha = {alpha:.2f} outside expected range [1, 8]")
        else:
            # Linear space checks
            if params.c0 <= 0:
                self.issues.append(f"[WARN] c0 = {params.c0:.4g} should be positive")
            if params.zeta < 0:
                self.issues.append(f"[WARN] zeta = {params.zeta:.4g} is negative (non-physical)")
            if params.tau_1ghz < 0:
                self.issues.append(f"[WARN] tau_1ghz = {params.tau_1ghz:.4g} is negative")
            alpha = getattr(params, 'alpha', 4.0)
            if alpha < 2 or alpha > 6:
                self.issues.append(f"[WARN] alpha = {alpha:.2f} outside typical range [2, 6]")
...
def create_data_overview_figure(dataset, output_path: Path, validator: FigureValidator):
    ...
    im = ax.imshow(dataset.data, aspect='auto', origin='lower',
                   extent=[dataset.time[0], dataset.time[-1],
                           dataset.freq[0], dataset.freq[-1]],
                   vmin=vmin, vmax=vmax, cmap='plasma')
    ax.set_xlabel('Time [ms]')
    ax.set_ylabel('Frequency [GHz]')
    ax.set_title(f'Dynamic Spectrum\n({dataset.data.shape[0]} ch × {dataset.data.shape[1]} bins)')
    ...
    ax.axvline(dataset.time[peak_idx], color='r', ls=':', alpha=0.7, label=f'Peak @ {dataset.time[peak_idx]:.2f} ms')
    ...
    fig.text(0.99, 0.01, sanity_text, fontsize=7, ha='right', va='bottom',
    ...
    print(f"    [SANITY] Data range: [{data_min:.3g}, {data_max:.3g}], Peak @ {peak_time:.2f} ms")


def create_initial_guess_figure(dataset, params: FRBParams, model_key: str, 
                                 output_path: Path, validator: FigureValidator):
    """
    ...
    # Data
    vmin, vmax = np.nanpercentile(dataset.data, [1, 99])
    im0 = axes[0, 0].imshow(dataset.data, aspect='auto', origin='lower',
                            extent=[dataset.time[0], dataset.time[-1],
                                   dataset.freq[0], dataset.freq[-1]],
                            vmin=vmin, vmax=vmax, cmap='plasma')
    ...
    fig.text(0.99, 0.02, sanity_text, fontsize=8, ha='right', va='bottom',
             family='monospace',
             bbox=dict(boxstyle='round', facecolor='lightgreen' if correlation > 0.5 else 'lightcoral', alpha=0.5))
    
    plt.tight_layout()
    fig.savefig(output_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"    Saved: {output_path}")
    print(f"    [SANITY] χ²={chi2:.4f}, Corr={correlation:.3f} {'✓' if correlation > 0.5 else '✗'}")


def create_results_summary_figure(results: dict, dataset, output_path: Path, 
    ...
    if best_params is not None:
        ...
        log_space = (best_params.c0 < 0 or best_params.zeta < 0 or best_params.tau_1ghz < 0)
    ...
    if flat_chain is not None and len(param_names) > 0:
        ...
        for i in range(n_params):
            ...
            ax.hist(samples, bins=50, alpha=0.5, color=colors[i], 
    ...
    if model_instance is not None and best_params is not None:
        ...
        ax.hist(res_flat, bins=100, density=True, alpha=0.7, color='gray', label='Residuals')
        ...
        if abs(res_mean) > 0.1:
            validator.issues.append(f"[WARN] Residual mean = {res_mean:.3f} (should be ≈ 0)")
        if abs(res_std - 1.0) > 0.3:
            validator.issues.append(f"[WARN] Residual std = {res_std:.3f} (should be ≈ 1)")
...
def main(config_file: str | None = None):
    ...
    create_data_overview_figure(
        pipe.dataset,
        output_dir / "01_data_overview.pdf",
        validator
    )
    ...
    try:
        ...
        create_initial_guess_figure(
            pipe.dataset, initial_params, "M3",
    ...
    create_results_summary_figure(
        ...
        output_dir / "03_results_summary.pdf",
        validator
    )
    ...
    print(f"  + Pipeline diagnostic plots (FRB_four_panel.pdf, FRB_comp_diagnostics.pdf)")
    print(f"{'='*60}\n")
