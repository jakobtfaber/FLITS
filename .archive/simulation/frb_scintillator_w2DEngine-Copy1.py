...
class Screen:
    ...
    def __init__(self, cfg: ScreenCfg, D_screen_m: float):
        ...
        if cfg.profile == "gaussian":
            ...
            # The screen size L is the 1/e radius (σ).
            sigma_rad = (L_m / self.D_screen_m)
...
class FRBScintillator:
    ...
    def _precompute_doppler_terms(self):
        ...
        The delay for any path is τ(t) = C + B*t + A*t^2, where C is the static
        delay, B is the linear (Doppler) term, and A is the quadratic term.
        """
        # C term: The static delay, τ(0)
        ...
        # This is the time derivative of τ evaluated at t=0.
        dtau_dv_mw = (self.deff_mw_m / self.C_M_PER_S) * (self.mw_screen.theta @ v_mw_mps.T)
        dtau_dv_host = (self.deff_host_m / self.C_M_PER_S) * (self.host_screen.theta @ v_host_mps.T)
        ...
        # This is 0.5 times the second time derivative of τ.
        d2tau_dv2_mw = (self.deff_mw_m / (2 * self.C_M_PER_S)) * np.sum(v_mw_mps**2)
        d2tau_dv2_host = (self.deff_host_m / (2 * self.C_M_PER_S)) * np.sum(v_host_mps**2)
        d2tau_dv2_cross = -(self.deff_mw_m / self.C_M_PER_S) * np.sum(v_mw_mps * v_host_mps)
        # This term is constant across all paths, so we broadcast it
