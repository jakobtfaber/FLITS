...
class FRBScintillator:
    ...
    def fit_acf(self, corr: np.ndarray, lags: np.ndarray) -> tuple[float, float]:
        ...
        and fits the residual to find the narrower component.

        This strategy is based on the analysis method described in
        Pradeep et al. (2025), where distinct scintillation components are
        ...
        # --- Edge Case: No significant scintillation ---
        if corr.size < 5 or corr[0] < 0.1:
            return np.nan, np.nan

        def lorentzian_model(x, amplitude, hwhm, eps=1e-12):
            """A single Lorentzian model."""
            # Add a small epsilon to avoid division by zero if hwhm is tiny
            return amplitude / (1 + (x / (hwhm + eps))**2)

        # --- Step 1: Fit the broad component ---
        # We assume the broad component dominates the "wings" of the ACF.
        # We exclude the very central part to avoid contamination from the narrow spike.
        broad_fit_start_idx = 5
        if len(lags_hz) <= broad_fit_start_idx:
            return np.nan, np.nan # Not enough data points

        x_broad, y_broad = lags_hz[broad_fit_start_idx:], corr[broad_fit_start_idx:]

        try:
            # Initial guess for the broad component's HWHM
            half_max_broad_idx = np.where(y_broad < y_broad[0] * 0.5)[0][0]
            w_broad_guess = x_broad[half_max_broad_idx]
        except IndexError:
            w_broad_guess = lags_hz[-1] / 2 # Fallback guess

        p0_broad = (y_broad[0], w_broad_guess) # Initial guess: (amplitude, hwhm)

        try:
            popt_broad, _ = curve_fit(lorentzian_model, x_broad, y_broad, p0=p0_broad, bounds=([0, 0], [np.inf, np.inf]))
        ...
        # We only fit the positive part of the residual to avoid fitting noise
        ...
        try:
            ...
            half_max_narrow_idx = np.where(y_narrow < y_narrow[0] * 0.5)[0][0]
            w_narrow_guess = x_narrow[half_max_narrow_idx]
        except (IndexError, KeyError):
            w_narrow_guess = self.dnu * 3 # Fallback guess

        p0_narrow = (y_narrow[0], w_narrow_guess)
        ...
        # Ensure broad > narrow
        if hwhm_broad < hwhm_narrow:
            hwhm_broad, hwhm_narrow = hwhm_narrow, hwhm_broad

        return hwhm_broad, hwhm_narrow
    ...
    def analyze_intra_pulse_scintillation(self, I_t_nu: np.ndarray, nbin_avg: int = 4) -> dict: #model: str = "composite") -> dict:
        """
        ...
        for i in trange(num_spectra, desc="Analyzing time slices"):
            ...
            nu_s_mw, nu_s_host = self.fit_acf(corr, lags)
    ...
    def get_irf_spikes(self) -> tuple[np.ndarray, np.ndarray]:
        """Returns the delays and intensities of individual geometric paths."""
    ...
    def simulate_scattered_time_series(self, time_res: u.Quantity, duration: u.Quantity, rng=None):
        ...
        the time-domain IRF with the intrinsic pulse.
        """
        if rng is None: rng = np.random.default_rng()
        ...
        delays, amps_sq = self.get_irf_spikes()
        amps = np.sqrt(amps_sq) * np.exp(2j * np.pi * rng.uniform(0, 1, amps_sq.size))
        irf_t = np.zeros(n_t, dtype=np.complex128)
        time_bins = np.round(delays / time_res_s).astype(int)
        valid_mask = (time_bins >= 0) & (time_bins < n_t)
        np.add.at(irf_t, time_bins[valid_mask], amps[valid_mask])
        
        t_pulse = (np.arange(n_t) - n_t//8) * time_res_s
        sigma_t = self.cfg.pulse_width.to(u.s).value / (2 * np.sqrt(2 * np.log(2)))
        envelope = np.exp(-t_pulse**2 / (2 * sigma_t**2))
        noise = rng.normal(size=n_t) + 1j * rng.normal(size=n_t)
        intrinsic_pulse_t = envelope * noise / np.sqrt(2)
