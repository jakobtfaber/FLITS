...
# Try to import shared logging helper, fallback to standard logging
try:
...
# Observatory coordinates as module-level constants for maintainability
CHIME_COORDINATES = {
    'lat': 49.3206 * u.deg, 
    'lon': -119.6236 * u.deg, 
    'height': 545 * u.m
...
def main(dm_opt=550.0, dm_uncertainty=0.2, dsa_mjd=59000.1, 
    ...
    dm_uncertainty : float, optional  
        DM uncertainty in pc/cm^3. Default: 0.2
    dsa_mjd : float, optional
        DSA-110 observation time in MJD. Default: 59000.1
    chime_unix_timestamp : float, optional
        CHIME observation timestamp (Unix time). Default: 1598882400.0
    source_coord : str, optional
        Source coordinates. Default: "12:00:00 +20:00:00"
    """
    ...
    logger.info("CHIME TOA Error due to DM uncertainty: %.3f ms", error_chime.value)
    logger.info("DSA-110 TOA Error due to DM uncertainty: %.3f ms", error_dsa.value)

    # --- Final Results ---
    dt = toa_400_utc_chime - toa_400_utc_dsa
    logger.info("Measured TOA Offset (Δt): %.3f ms", dt.to(u.ms).value)
    logger.info("Combined Uncertainty on Δt from DM: ±%.3f ms", delta_t_uncertainty.value)

    # Geometric delay calculation
    src = SkyCoord(source_coord, unit=(u.hourangle, u.deg), frame="icrs")
    chime_loc = EarthLocation(**CHIME_COORDINATES)
    dsa_loc = EarthLocation(**DSA_COORDINATES)

    geometric_delay = compute_geometric_delay(toa_400_utc_chime, src, chime_loc, dsa_loc)
    logger.info("Geometric Delay: %.3f ms", geometric_delay.value)
