...
# Observatory coordinates as module-level constants for reusability
...
def main(dm_opt=550.0, dm_uncertainty=0.2, dsa_mjd=59000.1, 
    ...
    """Analyze TOA crossmatch between observatories.
    
    Parameters
    ----------
    dm_opt : float, optional
        Optimal dispersion measure in pc/cm^3. Default: 550.0
    dm_uncertainty : float, optional
        Uncertainty in dispersion measure in pc/cm^3. Default: 0.2
    dsa_mjd : float, optional
        DSA-110 observation time in MJD. Default: 59000.1
    chime_unix_timestamp : float, optional
        CHIME observation time in Unix timestamp. Default: 1598882400.0
    source_coord : str, optional
        Source coordinates in "HH:MM:SS +DD:MM:SS" format. Default: "12:00:00 +20:00:00"
    """
    ...
    # Improved logging format with explicit units for clarity
    logger.info("CHIME TOA Error due to DM uncertainty: %.3f ms", error_chime.to(u.ms).value)
    logger.info("DSA-110 TOA Error due to DM uncertainty: %.3f ms", error_dsa.to(u.ms).value)

    # --- Final Results ---
    dt = toa_400_utc_chime - toa_400_utc_dsa
    logger.info("Measured TOA Offset (Δt): %.3f ms", dt.to(u.ms).value)
    logger.info("Combined Uncertainty on Δt from DM: ±%.3f ms", delta_t_uncertainty.to(u.ms).value)

    # Geometric delay calculation using module-level constants
    src = SkyCoord(source_coord, unit=(u.hourangle, u.deg), frame="icrs")
    geometric_delay_value = compute_geometric_delay(toa_400_utc_chime, src, CHIME_LOCATION, DSA_LOCATION)
    logger.info("Geometric Delay: %.3f ms", geometric_delay_value.to(u.ms).value)
