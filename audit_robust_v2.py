...
def audit_file(file_path):
    ...
    for i in range(4):
        ...
        if np.max(prof) - np.min(prof) < 0.1: # Threshold for noise
            continue
        ...
        # Calculate centroid around peak to be safer than just argmax
        w = 500
    ...
    # Arrival time t \propto nu^-2. 
    # If slope > 0, then higher index arrives LATER -> higher index is LOW freq -> DESCENDING
    # If slope < 0, then higher index arrives EARLIER -> higher index is HIGH freq -> ASCENDING
    
    res = "Ascending" if slope < 0 else "Descending"
    return slope, res

def run_audit():
    chime_dir = "data/chime"
    files = [f for f in os.listdir(chime_dir) if f.endswith(".npy")]
    
    print(f"{'Filename':<50} | {'Slope':>10} | {'Status':<10}")
    print("-" * 75)
    
    for f in files:
        path = os.path.join(chime_dir, f)
