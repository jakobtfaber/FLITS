...
class FRBScintillator:
    ...
    def synthesise_dynamic_spectrum(self, duration: u.Quantity, dt_epoch: u.Quantity = 0.0 * u.s, rng=None):
        """
        Generates a full 2D dynamic spectrum I(t, ν) via time-domain convolution
        and subsequent channelization (FFT-and-square).
        ...
        Returns:
            ...
            spectrum (time, freq), the time axis (s), and the frequency axis (Hz).
        """
        if rng is None: rng = np.random.default_rng()
        
        # The underlying time resolution must satisfy Nyquist for the bandwidth
        fs = self.bw_hz
        time_res_s = 1.0 / fs
        n_t = int(duration.to(u.s).value / time_res_s)
        
        # 1. Get time-domain IRF and intrinsic pulse
        irf_t = self._get_time_domain_irf(dt_epoch.to(u.s).value, n_t, time_res_s)
        pulse_t = self._get_intrinsic_pulse(n_t, time_res_s, rng)
        
        # 2. Convolve in frequency domain to get observed E-field vs. time
        E_obs_t = np.fft.ifft(np.fft.fft(irf_t) * np.fft.fft(pulse_t))
        
        # 3. Manually channelize using the "FFT-and-square" method
        ...
        # 4. Create the corresponding time and frequency axes for the output
