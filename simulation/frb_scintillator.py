...
# Setup logging
log = logging.getLogger("frb_scintillator")

try:
    import numba as nb
    _NUMBA = True
    log.info("Numba detected. Using JIT-accelerated version.")
except ModuleNotFoundError:
    log.info("Numba not found, using pure Python loops.")
    _NUMBA = False
...
if _NUMBA:
    ...
    def _irf_coherent_numba_loop(field_products, total_delay, freqs):
        """
        Vectorised IRF: builds the full phase matrix once, then sums over
        propagation paths -- greatly reducing memory traffic.
        """
        phase = np.exp(-2j * np.pi * total_delay[:, None] * freqs[None, :])
        # Broadcasting: (N_path, N_chan) * (N_path, 1) → (N_path, N_chan)
        field_mtx = field_products[:, None] * phase
        return np.sum(field_mtx, axis=0)       # (N_chan,)
...
# -------------------------------------------------------------------------
# Instrumental parameters (all optional; default = noise-free)
# -------------------------------------------------------------------------
@dataclass
class InstrumentalCfg:
    ...
    def get_sefd_jy(self) -> float | None:
        """
        Returns SEFD in Jy, calculating from T_sys/A_eff if needed.
        """
...
# -------------------------------------------------------------------------
# Single Thin Scattering Screen parameters
# -------------------------------------------------------------------------
@dataclass
class ScreenCfg:
    """
    ...
    Attributes:
        ...
        L (u.Quantity): Characteristic physical size (1-sigma radius, σ) of the screen.
...
class Screen:
    ...
    def __init__(self, cfg: ScreenCfg, D_screen_m: float):
        ...
        # The angular size (1-sigma radius) of the screen in radians
        sigma_rad = L_m / D_screen_m
        
        # --- Generate image coordinates (theta) ---
        # Start with a circular distribution and then scale it
        if cfg.geometry == "1D":
            xy = np.zeros((cfg.N, 2))
            # For a 1D screen, we place all images along the x-axis before rotation
        ...
        # Store the scaling vector and apply it to the coordinates to create
        # an elliptical distribution of scatterers.
        ...
        if cfg.pa.value != 0.0:
            ...
            xy = xy @ R.T
        self.theta = xy # Final angular positions of images [rad]
        ...
        if cfg.profile == "gaussian":
            # Un-rotate and un-scale to calculate the envelope in the screen's native frame
            if cfg.pa.value != 0.0:
            ...
            # (Power-law profile is assumed isotropic for now)
...
class FRBScintillator:
    ...
    def synthesise_dynamic_spectrum_2d(self, duration: u.Quantity, time_res: u.Quantity, rng=None):
        ...
        log.info("Running True 2D Engine (this may take a while)...")
    ...
    def synthesise_dynamic_spectrum_2d_parallel(self, duration: u.Quantity, time_res: u.Quantity, rng=None):
        ...
        log.info("Running Parallel True 2D Engine...")
    ...
    def analyze_intra_pulse_scintillation(self, I_t_nu: np.ndarray, time_axis: np.ndarray) -> dict:
        ...
        log.info("Analyzing scintillation evolution across the pulse...")
